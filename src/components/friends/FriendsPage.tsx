...
export const FriendsPage: React.FC<FriendsPageProps> = ({
  ...
  const handleAddSuggested = (suggested: SuggestedFriend) => {
    ...
    const newFriend: FriendUser = {
      ...
      isFriend: true,
      tags: [suggested.sharedInterest.replace('Similar goals: ', '')],
    };
    setFriends((prev) => [...prev, newFriend]);
    showToast(`Added ${suggested.name} to friends!`);
  };

  const handleAddFriendFromModal = (user: Partial<FriendUser>) => {
    const newFriend: FriendUser = {
      id: `f-${Date.now()}`,
  ...
  return (
    <div id="friends-page-container" className="flex-1 flex flex-col min-w-0 pb-24 lg:pb-12 bg-[#090B0F] text-[#F7F8FC]">
      {/* 1. TOP BAR */}
      <header 
        id="friends-top-bar"
      ...
      <main className="flex-1 px-4 sm:px-6 lg:px-8 py-6 max-w-[1240px] w-full mx-auto space-y-6">
        ...
        {/* HEADER SECTION */}
        <div id="friends-header-section" className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <h1 className="text-2xl sm:text-3xl font-bold text-white tracking-tight">
              Friends
            </h1>
            <p className="text-xs sm:text-sm text-[#9AA3B5] mt-1">
          ...
          <div className="flex items-center gap-4">
            ...
            <p className="hidden xl:block text-xs italic text-[#9AA3B5] max-w-[260px] text-right font-serif">
            ...
            <button 
              ...
              className="h-10 px-4 rounded-xl bg-[#6366F1] hover:bg-[#7C7FF5] text-white text-xs sm:text-sm font-semibold flex items-center gap-2 transition-all shadow-md shadow-indigo-600/25 shrink-0"
            >
        ...
        {/* TABS NAVIGATION */}
        <div className="flex items-center gap-2 overflow-x-auto pb-1 scrollbar-none border-b border-white/6 pt-1">
          {[
            { id: 'overview', label: 'Overview' },
            { id: 'my-friends', label: `My Friends`, count: friends.length },
            { id: 'requests', label: 'Requests', count: requests.length, isDangerBadge: true },
