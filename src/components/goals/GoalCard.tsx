...
export const GoalCard: React.FC<GoalCardProps> = ({
  ...
  return (
    <div
      ...
      className="group relative flex flex-col justify-between bg-[#141821] hover:bg-[#181D27] border border-white/8 hover:border-white/16 rounded-[14px] p-4 sm:p-5 transition-all duration-200 cursor-pointer shadow-[0_4px_20px_rgba(0,0,0,0.18)] hover:-translate-y-0.5 select-none"
    >
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="flex flex-col min-w-0">
            <h3 className="text-sm sm:text-base font-semibold text-[#F7F8FC] group-hover:text-white truncate">
              {goal.title}
            </h3>
            <span
        ...
        <div className="relative shrink-0" onClick={(e) => e.stopPropagation()}>
          <button
            ...
            className="w-7 h-7 rounded-lg flex items-center justify-center text-[#697388] hover:text-[#F7F8FC] hover:bg-white/5 transition-colors"
          ...
          {/* Three-dot Dropdown */}
          {showMenu && (
            <div className="absolute right-0 top-8 w-44 bg-[#181D27] border border-white/12 rounded-xl shadow-2xl py-1.5 z-30 animate-in fade-in zoom-in-95 duration-100">
              <button
                onClick={() => {
                  setShowMenu(false);
                  onSelect(goal);
                }}
                className="w-full text-left px-3 py-1.5 text-xs text-[#A5AEC2] hover:text-white hover:bg-white/5 flex items-center gap-2"
              >
      ...
      <p className="text-xs text-[#A5AEC2] mt-3 line-clamp-2 leading-relaxed min-h-[36px]">
        {goal.description}
      </p>
      ...
      <div className="mt-4">
        <div className="flex items-center justify-between text-xs mb-1.5">
          <div className="flex-1 h-2 bg-[#0E1118] rounded-full overflow-hidden mr-3">
            <div
          ...
          <span className="text-xs font-semibold text-[#F7F8FC] tabular-nums shrink-0">
            {goal.progress}%
          </span>
        </div>
      </div>
      ...
      <div className="flex items-center justify-between text-[11px] text-[#697388] mt-3 pt-3 border-t border-white/6">
        ...
        {goal.collaborator && (
          <div
            className="w-5 h-5 rounded-full overflow-hidden border border-white/10 shrink-0"
