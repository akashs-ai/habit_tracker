...
import { Sparkles, Calendar, Code2, Users, Github } from 'lucide-react';
  ...
  useEffect(() => {
    ...
    TEAM_MEMBERS.forEach(async (member) => {
      ...
      try {
        ...
        if (isMounted) {
          setMemberStatsMap((prev) => ({ ...prev, [member.id]: stats }));
        }
      } catch {
          ...
          <div className="flex flex-col md:flex-row items-center justify-between gap-4 md:gap-6">
            ...
            <div className="flex flex-col sm:flex-row items-center gap-3 text-center sm:text-left">
              <div className="flex items-center gap-2 px-3 py-1.5 rounded-xl bg-indigo-50/80 dark:bg-[#6366F1]/10 border border-indigo-200/70 dark:border-[#6366F1]/25 text-indigo-700 dark:text-[#A5B4FC]">
                <div className="w-5 h-5 rounded-lg bg-[#6366F1] flex items-center justify-center text-white shadow-xs">
            ...
            {/* Center: Developed by Interactive Member Names (Auto-synced) */}
            <div className="flex flex-col items-center gap-1.5 text-center">
              ...
              <div className="flex flex-wrap items-center justify-center gap-1.5 sm:gap-2">
                {TEAM_MEMBERS.map((member, index) => {
                  const hasStats = Boolean(memberStatsMap[member.id]);
                  ...
                  return (
                    <React.Fragment key={member.id}>
                      <button
                        ...
                        title={`View ${member.name} (${member.role}) profile${hasStats ? ` · ${memberStatsMap[member.id].publicRepos} repos auto-synced` : ''}`}
