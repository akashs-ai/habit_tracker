...
export const TimeRangeControls: React.FC<TimeRangeControlsProps> = ({
  ...
  return (
    ...
    >
      {/* Time Range Pills */}
      <div className="flex items-center gap-1.5 overflow-x-auto pb-1 sm:pb-0 scrollbar-none bg-[#0F1723]/90 p-1 rounded-xl border border-white/6 w-fit">
        {ranges.map((r) => {
      ...
      <button
        ...
        className="h-9 px-3.5 rounded-xl bg-[#0F1723] hover:bg-[#141C2B] border border-white/6 text-xs text-[#94A3B8] hover:text-white flex items-center justify-between sm:justify-start gap-2 transition-colors cursor-pointer self-start sm:self-auto shrink-0 shadow-sm"
        title="Change custom date range"
      >
        <div className="flex items-center gap-2">
          <Calendar className="w-3.5 h-3.5 text-[#818CF8]" />
          <span className="font-medium text-white/90">{dateRangeText}</span>
        </div>
