...
export const AnalyticsPage: React.FC<AnalyticsPageProps> = ({
  ...
  return (
    <div 
      ...
      className="flex-1 flex flex-col min-w-0 bg-slate-50 dark:bg-[#080F1A] text-slate-900 dark:text-[#F1F5F9] font-sans pb-24 lg:pb-12 min-h-screen transition-colors duration-200"
    >
      {/* 1. Header Bar matching Desktop (72px) & Tablet/Mobile (58-64px) */}
      <header 
        id="analytics-top-header"
        className="h-16 lg:h-[72px] px-4 sm:px-6 lg:px-8 border-b border-slate-200 dark:border-white/6 flex items-center justify-between gap-4 sticky top-0 bg-white/90 dark:bg-[#080F1A]/95 backdrop-blur-md z-20 transition-colors"
      >
        ...
        <div className="flex items-center gap-3 flex-1 max-w-xl">
          ...
          <button
            ...
            className="lg:hidden p-2 rounded-xl text-slate-500 dark:text-[#94A3B8] hover:text-slate-900 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-white/5 border border-slate-200 dark:border-white/6 transition-colors"
          ...
          <div className="flex items-center gap-2 lg:hidden">
            <div className="w-7 h-7 rounded-lg bg-gradient-to-tr from-[#6366F1] to-[#8B5CF6] flex items-center justify-center text-white text-xs font-bold shadow-xs">
              L
            </div>
            <span className="font-bold text-sm tracking-tight text-slate-900 dark:text-white hidden sm:inline">LifeRPG</span>
          </div>

          {/* Search Input Box */}
          <div className="relative w-full max-w-md hidden sm:block">
            <Search className="w-4 h-4 text-slate-400 dark:text-[#64748B] absolute left-3.5 top-1/2 -translate-y-1/2 pointer-events-none" />
            <input
              type="text"
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              placeholder="Search tasks, goals, or anything..."
              className="w-full h-10 pl-9.5 pr-14 rounded-xl bg-slate-100 dark:bg-[#0F1723] border border-slate-200 dark:border-white/7 text-xs text-slate-900 dark:text-white placeholder-slate-400 dark:placeholder-[#64748B] focus:outline-none focus:border-[#6366F1] focus:ring-1 focus:ring-[#6366F1] transition-all"
            />
            <div className="absolute right-2.5 top-1/2 -translate-y-1/2 flex items-center gap-1 text-[11px] font-mono text-slate-500 dark:text-[#64748B] bg-slate-200/80 dark:bg-white/4 border border-slate-300 dark:border-white/7 px-1.5 py-0.5 rounded pointer-events-none">
              <span>⌘</span>
              <span>K</span>
            </div>
          </div>
        ...
        <div className="flex items-center gap-2.5 sm:gap-3 shrink-0">
          ...
          <button
            ...
            className="p-2 sm:p-2.5 rounded-xl text-slate-500 dark:text-[#94A3B8] hover:text-slate-900 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-white/5 border border-slate-200 dark:border-white/6 transition-colors"
          ...
          <div className="relative">
            ...
            <span className="absolute top-2 right-2 w-2 h-2 rounded-full bg-[#EF4444] ring-2 ring-white dark:ring-[#080F1A]" />
          </div>
          ...
          <div 
            ...
            title="Alex (Level 12)"
          >
            <div className="w-full h-full rounded-full bg-slate-100 dark:bg-[#1E293B] flex items-center justify-center text-xs font-bold text-slate-800 dark:text-white">
              A
            </div>
          </div>
        </div>
      ...
      {activeDetailModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 dark:bg-black/75 backdrop-blur-xs animate-in fade-in duration-150">
          <div 
            className="w-full max-w-lg bg-white dark:bg-[#0F1723] border border-slate-200 dark:border-white/10 rounded-2xl shadow-2xl p-5 sm:p-6 text-slate-900 dark:text-white relative animate-in zoom-in-95 duration-150"
          ...
          >
            <div className="flex items-center justify-between pb-4 border-b border-slate-200 dark:border-white/8">
              ...
              <button
                ...
                className="p-1 rounded-lg text-slate-400 dark:text-[#94A3B8] hover:text-slate-900 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-white/5 transition-colors"
              >
                <X className="w-5 h-5" />
              </button>
            </div>

            <div className="py-4 text-xs sm:text-sm text-slate-600 dark:text-[#94A3B8] space-y-3">
              {activeDetailModal === 'daterange' && (
                <div>
                  <p className="text-slate-900 dark:text-white font-medium mb-3">Select a date range preset or pick custom dates:</p>
                  <div className="grid grid-cols-2 gap-2">
                    {['This Week', 'Last Week', 'This Month', 'Last 30 Days', 'Last Quarter', 'Year to Date'].map((preset) => (
                      <button
                        ...
                        className="p-2.5 rounded-xl bg-slate-100 dark:bg-white/4 hover:bg-[#6366F1] hover:text-white dark:hover:bg-[#6366F1] dark:hover:text-white border border-slate-200 dark:border-white/6 text-left transition-colors font-medium text-xs text-slate-800 dark:text-white/90"
                      >
                        {preset}
                      </button>
                    ))}
              ...
              {activeDetailModal === 'consistency' && (
                <div className="space-y-2 text-xs">
                  <p className="text-slate-900 dark:text-white font-semibold text-sm">Overall Consistency: 82%</p>
                  <p>Calculated across all tracked daily habits and planned task check-ins during the current evaluation period.</p>
                  <div className="p-3 rounded-xl bg-slate-50 dark:bg-white/4 border border-slate-200 dark:border-white/6 mt-2">
                    <p className="text-emerald-600 dark:text-[#34D399] font-medium">↑ 12% improvement over previous 7-day cycle.</p>
                    <p className="text-slate-500 dark:text-[#94A3B8] mt-1">Best performing day: Thursday (92% completion).</p>
                  </div>
                </div>
              )}
              ...
              {activeDetailModal === 'streak' && (
                <div className="space-y-2 text-xs">
                  <p className="text-slate-900 dark:text-white font-semibold text-sm">Current Active Streak: 6 Days</p>
                  <p>You have logged into LifeRPG and fulfilled your baseline daily quota 6 days in a row without missing a single day.</p>
                  <p className="text-emerald-600 dark:text-[#34D399] font-medium">Only 1 day left to unlock the 7-Day Consistency Master milestone!</p>
                </div>
              )}

              {activeDetailModal === 'tasks' && (
                <div className="space-y-2 text-xs">
                  <p className="text-slate-900 dark:text-white font-semibold text-sm">Tasks Completed: 28 of 34 (82.3%)</p>
              ...
              {activeDetailModal === 'momentum' && (
                <div className="space-y-2 text-xs">
                  <p className="text-slate-900 dark:text-white font-semibold text-sm">4,320 Momentum Points</p>
                  <p>Momentum Points reflect your ongoing productivity cadence and habit consistency over time.</p>
                  <p className="text-indigo-600 dark:text-[#A78BFA] font-medium">+540 MP earned this current week.</p>
                </div>
              )}
              ...
              {activeDetailModal === 'notifications' && (
                <div className="space-y-2 text-xs">
                  <div className="p-2.5 rounded-lg bg-slate-50 dark:bg-white/4 border border-slate-200 dark:border-white/6">
                    <p className="text-slate-900 dark:text-white font-medium">Weekly Analytics Summary Ready</p>
                    <p className="text-[11px] text-slate-500 dark:text-[#94A3B8] mt-0.5">Your consistency rose by +12% compared to last week.</p>
                  </div>
                  <div className="p-2.5 rounded-lg bg-slate-50 dark:bg-white/4 border border-slate-200 dark:border-white/6">
                    <p className="text-slate-900 dark:text-white font-medium">Streak Alert</p>
                    <p className="text-[11px] text-slate-500 dark:text-[#94A3B8] mt-0.5">You are on a 6-day streak! Don&apos;t forget your evening reading.</p>
                  </div>
                </div>
              )}
              ...
              {activeDetailModal === 'profile' && (
                <div className="space-y-2 text-xs">
                  <p className="text-slate-900 dark:text-white font-bold text-sm">Alex • Productivity Specialist</p>
                  <p className="text-slate-700 dark:text-slate-200">Current Level: 12 • Total Momentum Points: 4,320 MP</p>
                  <p className="text-slate-500 dark:text-[#94A3B8]">Productivity profile configured with strict habit tracking cadence.</p>
                </div>
              )}
            </div>

            <div className="pt-3 border-t border-slate-200 dark:border-white/8 flex justify-end">
              <button
                onClick={() => setActiveDetailModal(null)}
