    ...
    >
        ...
        {categories.map((cat) => {
          ...
          return (
            <button
              ...
              className={`h-9 px-3.5 rounded-xl text-xs font-semibold flex items-center gap-2 whitespace-nowrap transition-all duration-150 ${
                isActive
                  ? 'bg-[#6366F1] text-white shadow-sm shadow-indigo-600/30'
                  : 'bg-white/4 text-[#9AA3B5] hover:bg-white/8 hover:text-white border border-white/5'
              }`}
            >
              {Icon && <Icon className={`w-3.5 h-3.5 ${isActive ? 'text-white' : 'text-[#818CF8]'}`} />}
              <span>{cat.label}</span>
            </button>
          );
        })}
      ...
      <div className="flex items-center gap-2 shrink-0">
        <div className="relative flex-1 md:w-56">
          <Search className="w-3.5 h-3.5 text-[#687185] absolute left-3 top-2.5" />
          <input
            type="text"
            value={searchQuery}
            onChange={(e) => onSearchChange(e.target.value)}
            placeholder="Search rewards..."
            className="w-full h-9 pl-9 pr-3 rounded-xl bg-[#121620] border border-white/8 text-xs text-white placeholder:text-[#687185] focus:outline-none focus:border-[#6366F1] transition-all"
          />
        </div>

        <button
          onClick={onToggleFilterModal}
          className="h-9 px-3 rounded-xl bg-white/4 border border-white/8 hover:bg-white/8 text-xs font-semibold text-[#9AA3B5] hover:text-white flex items-center gap-1.5 transition-colors shrink-0"
        >
