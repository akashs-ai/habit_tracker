...
export const UpcomingEventsPanel: React.FC<UpcomingEventsPanelProps> = ({
  ...
  return (
    <div id="upcoming-events-panel" className="flex flex-col gap-2.5">
        ...
        {upcomingList.map((evt) => {
          ...
          return (
            <div
              ...
              className="flex items-center gap-3 p-2.5 rounded-lg hover:bg-[#151820] cursor-pointer transition-colors group"
            >
              {/* Icon */}
              <div 
              ...
              <div className="flex flex-col min-w-0 flex-1">
                <span className="text-xs font-medium text-[#F5F7FF] truncate group-hover:text-white transition-colors">
                  {evt.title}
                </span>
      ...
      <button
        onClick={onViewAll}
        className="flex items-center gap-1 text-xs text-[#6C63FF] hover:text-[#7B73FF] font-medium px-1 py-1 w-fit transition-colors mt-0.5"
      >
        <span>View All</span>
